...
public class Skidmarks : MonoBehaviour {
	...
	public float markWidth = 0.3f;		// The width of the skidmarks. Should match the width of the wheel that it is used for. In meters.
	...
	void Start1()
	{
		if (Network.isServer)
		{
			transform.position = new Vector3(-1, 0, -1.5f);
		}
	}
	...
	IEnumerator  AddSkidMarkDelay(Vector3 pos, Vector3  normal, float intensity, int lastIndex)
	{
		yield return new WaitForSeconds (0.35f);
		AddSkidMark (pos, normal, intensity, lastIndex);
	}
	
	[RPC]
	public void sendSkidmarkInfor(Vector3 pos, Vector3  normal, float intensity, int lastIndex)
	{
		if (Network.isServer)
		{
			//AddSkidMark (pos, normal, intensity, lastIndex);
			StartCoroutine(AddSkidMarkDelay(pos, normal, intensity, lastIndex));
		}
	}

	public int AddSkidMarkTruck(Vector3 pos, Vector3  normal, float intensity, int lastIndex)
	{
		if (Network.isClient)
		{
			transform.networkView.RPC("sendSkidmarkInfor", RPCMode.AllBuffered, pos, normal, intensity, lastIndex);
		}

		return AddSkidMark (pos, normal, intensity, lastIndex);
	}
